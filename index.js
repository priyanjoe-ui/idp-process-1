...
async function checkQueueHealth() {
  const listResp = await sqsClient.send(new ListQueuesCommand({ QueueNamePrefix: 'idp' }));
  const queueUrls = listResp.QueueUrls || [];
  if (queueUrls.length === 0) {
    return { service: 'queue', label: 'Intake Queue', status: 'info', detail: 'No queue found (looked for names starting with "idp").' };
  }

  const results = await Promise.all(queueUrls.map(async url => {
  ...
  const totalVisible = results.reduce((sum, r) => sum + r.visible, 0);
  let status = 'healthy';
  if (totalVisible >= HEALTH_THRESHOLDS.queueStalled) status = 'down';
  else if (totalVisible >= HEALTH_THRESHOLDS.queueBackingUp) status = 'degraded';

  const detail = results.length === 1
    ? `${results[0].visible} message${results[0].visible === 1 ? '' : 's'} waiting (${results[0].name})`
    : results.map(r => `${r.name}: ${r.visible}`).join(' · ');

  return { service: 'queue', label: 'Intake Queue', status, detail };
}
...
async function checkLambdaHealth() {
  const functions = await _listIdpFunctions();
  if (functions.length === 0) {
    return { service: 'lambda', label: 'Lambda Functions', status: 'info', detail: 'No functions found (looked for names starting with "idp-").' };
  }

  let down = 0;
  let degraded = 0;
  for (const fn of functions) {
    if (fn.State && fn.State !== 'Active') { down++; continue; }
    try {
      const [errors, invocations] = await Promise.all([
        _sumMetric('AWS/Lambda', 'Errors', [{ Name: 'FunctionName', Value: fn.FunctionName }]),
        _sumMetric('AWS/Lambda', 'Invocations', [{ Name: 'FunctionName', Value: fn.FunctionName }]),
      ]);
      if (invocations > 0 && (errors / invocations) * 100 >= HEALTH_THRESHOLDS.lambdaErrorDegradedPct) degraded++;
    } catch {
      // Metric fetch failing doesn't mean the function is unhealthy — just
      // means we can't confirm either way, so it's not counted as degraded.
    }
  }
  let status = 'healthy';
...
/** Lambda's ListFunctions has no server-side name-prefix filter (unlike
 *  SQS's ListQueues), so this pages through every function in the account
 *  and filters client-side by the "idp-" naming convention used throughout
 *  this project. The response already includes each function's State, so
 *  no separate GetFunctionConfiguration call is needed per function. */
async function _listIdpFunctions() {
  const matches = [];
  let Marker;
  do {
    const resp = await lambdaClient.send(new ListFunctionsCommand({ Marker }));
    for (const fn of resp.Functions || []) {
      if (fn.FunctionName && fn.FunctionName.startsWith('idp-')) matches.push(fn);
    }
