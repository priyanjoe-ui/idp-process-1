...
app.post('/api/batches/:queueId/save-draft', async (req, res) => {
  ...
  async function respondVersionConflict(res, kind, id) {
    ...
    if (kind === 'APPLICATION') {
      ...
      const [appId, workflowId] = String(id).split('/');
      const row = await ddbConfigGet(`APP#${appId}`, `WORKFLOW#${workflowId}`);
      currentVersion = row?.version ?? null;
    }
  ...
  app.post('/api/applications/:appId/document-types', async (req, res) => {
    try {
      const { appId } = req.params;
      const { docTypeId, name, body } = req.body || {};
      ...
      const dtBody = body || {
        ...
        PageTypes: [
          { PageType: 'MainPage',  IdentificationMethod: 'Sequential', SequenceOrder: 1, Fields: [] },
          { PageType: 'BlankPage', IdentificationMethod: 'WordCount',  SequenceOrder: 2, MaxWordCount: 5 },
        ],
      };
      const item = {
        PK:     `APP#${appId}`,
  ...
  // Row shape:
  //   PK              SK                   type       body
  //   "APP#<appId>"   "WORKFLOW#<wfId>"    WORKFLOW   { name, definition, status, stateMachineArn? }
  //
  ...
  // lives in Step Functions itself, not duplicated here. This achieves
  // the same safety property (editing never touches the live pipeline)
  // with one row instead of two.
  //
  ...
  function toWorkflowSummary(row) {
    const appId      = appIdFromPK(row.PK);
    const workflowId = String(row.SK).replace(/^WORKFLOW#/, '');
    const body       = row.body || {};
  ...
  async function ddbConfigListWorkflows(appId) {
    const items = [];
    let ExclusiveStartKey;
    do {
      const resp = await ddb.send(new QueryCommand({
        ...
        ExpressionAttributeValues: { ':pk': `APP#${appId}`, ':wf': 'WORKFLOW#' },
        ExclusiveStartKey,
      }));
      items.push(...(resp.Items || []));
      ExclusiveStartKey = resp.LastEvaluatedKey;
  ...
  app.get('/api/applications/:appId/workflows', async (req, res) => {
    try {
      const rows = await ddbConfigListWorkflows(req.params.appId);
      const workflows = rows.map(toWorkflowSummary);
      workflows.sort((a, b) => a.name.localeCompare(b.name));
      res.json({ workflows });
    } catch (e) {
      console.error('GET /api/applications/:appId/workflows error:', e);
      jsonError(res, 500, 'InternalError', e.message);
    }
  });

  // Global list — used by the /workflows Studio route (sidebar "Jobs & Workflows").
  app.get('/api/workflows', async (_req, res) => {
    try {
      const rows = await ddbConfigQueryByType('WORKFLOW');
      const workflows = rows.map(toWorkflowSummary);
      workflows.sort((a, b) => a.name.localeCompare(b.name));
      res.json({ workflows });
    } catch (e) {
  ...
  app.get('/api/applications/:appId/workflows/:workflowId', async (req, res) => {
    try {
      const { appId, workflowId } = req.params;
      const row = await ddbConfigGet(`APP#${appId}`, `WORKFLOW#${workflowId}`);
      if (!row) return jsonError(res, 404, 'NotFound', `Workflow ${appId}/${workflowId} not found`);
      res.json(toWorkflowRecord(row));
    } catch (e) {
      console.error('GET /api/applications/:appId/workflows/:workflowId error:', e);
      jsonError(res, 500, 'InternalError', e.message);
    }
  });

  app.post('/api/applications/:appId/workflows', async (req, res) => {
    try {
      const { appId } = req.params;
      ...
      const existing = await ddbConfigGet(`APP#${appId}`, `WORKFLOW#${workflowId}`);
      if (existing) {
        return jsonError(res, 409, 'AlreadyExists', `Workflow ${workflowId} already exists in ${appId}`);
      }
      ...
      const item = {
        ...
        type: 'WORKFLOW',
        body: { name: name || workflowId, definition, status: 'draft' },
      };
      const written = await ddbConfigPutRow(item, null);
      res.status(201).json(toWorkflowRecord(written));
    } catch (e) {
      console.error('POST /api/applications/:appId/workflows error:', e);
      jsonError(res, 500, 'InternalError', e.message);
    }
  });

  // Save draft — writes to DynamoDB ONLY. Never calls Step Functions.
  app.put('/api/applications/:appId/workflows/:workflowId', async (req, res) => {
    try {
      const { appId, workflowId } = req.params;
      const expected = requireExpectedVersion(req, res);
      if (expected == null) return;
      const existing = await ddbConfigGet(`APP#${appId}`, `WORKFLOW#${workflowId}`);
      if (!existing) return jsonError(res, 404, 'NotFound', `Workflow ${appId}/${workflowId} not found`);

      const { definition } = req.body || {};
      if (!definition || !definition.StartAt || !definition.States) {
        return jsonError(res, 400, 'BadRequest', 'definition (with StartAt and States) is required.');
      }
      ...
      if (e.code === 'VersionConflict') {
        return respondVersionConflict(res, 'WORKFLOW', `${req.params.appId}/${req.params.workflowId}`);
      }
      console.error('PUT /api/applications/:appId/workflows/:workflowId error:', e);
      jsonError(res, 500, 'InternalError', e.message);
    }
  });

  app.delete('/api/applications/:appId/workflows/:workflowId', async (req, res) => {
    try {
      const { appId, workflowId } = req.params;
      await ddbConfigDelete(`APP#${appId}`, `WORKFLOW#${workflowId}`);
      res.status(204).end();
    } catch (e) {
      console.error('DELETE /api/applications/:appId/workflows/:workflowId error:', e);
      jsonError(res, 500, 'InternalError', e.message);
    }
  });
  ...
  app.post('/api/applications/:appId/workflows/:workflowId/deploy', async (req, res) => {
    try {
      const { appId, workflowId } = req.params;
      const expected = requireExpectedVersion(req, res);
      if (expected == null) return;
      ...
      if (existing.version !== expected) {
        return respondVersionConflict(res, 'WORKFLOW', `${appId}/${workflowId}`);
      }
      ...
      if (body.stateMachineArn) {
        ...
        const stateMachineName = `idp-${appId}-${workflowId}`.toLowerCase().slice(0, 80);
      ...
      console.error('POST /api/applications/:appId/workflows/:workflowId/deploy error:', e);
